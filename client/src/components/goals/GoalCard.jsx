    ...
    >
      ...
      <div className="grid grid-cols-1 gap-2 text-sm text-slate-200 sm:grid-cols-2">
        <p className="text-slate-600 dark:text-slate-200">
          Remaining: <span className="font-semibold">{calculator.years || 0} years and {calculator.remaining_months || 0} months</span>
        </p>
        <p className="text-slate-600 dark:text-slate-200">Monthly savings: <span className="font-semibold">{formatMad(goal.monthlySavings)}</span></p>
        <p className="text-slate-600 dark:text-slate-200">Daily savings: <span className="font-semibold">{formatMad(calculator.daily_needed)}</span></p>
        <p className="text-slate-600 dark:text-slate-200">Progress: <span className="font-semibold">{progress.toFixed(1)}%</span></p>
        <p className="text-slate-600 dark:text-slate-200">Completion date: <span className="font-semibold">{calculator.estimated_end_date ? new Date(calculator.estimated_end_date).toLocaleDateString("en-US") : "-"}</span></p>
      </div>

      <div className="mt-3 rounded-md bg-slate-50 p-3 text-sm dark:bg-slate-950">
        {isImpactLoading ? <p className="text-slate-500 dark:text-slate-400">Calculating loss impact...</p> : null}
        {!isImpactLoading && impactData.is_impossible ? (
          <p className="font-semibold text-rose-500">It is impossible to reach this goal with the current losses.</p>
        ) : null}
        {!isImpactLoading && !impactData.is_impossible ? (
          <p className="text-amber-600 dark:text-amber-300">
            Delay caused by losses: <span className="font-semibold">{impactData.delay_months || 0} months</span>
          </p>
        ) : null}
        ...
        {isAchieved ? <p className="font-semibold text-emerald-400">Goal achieved, great job 🎉</p> : null}
      </div>
