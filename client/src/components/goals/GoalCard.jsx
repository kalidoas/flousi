  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-2 text-sm text-slate-200 sm:grid-cols-2">
        ...
        <p className="text-slate-600 dark:text-slate-200">تاريخ التمام: <span className="font-semibold">{calculator.estimated_end_date ? new Date(calculator.estimated_end_date).toLocaleDateString("fr-MA") : "-"}</span></p>
      </div>

      <div className="mt-3 rounded-md bg-slate-50 p-3 text-sm dark:bg-slate-950">
        {isImpactLoading ? <p className="text-slate-500 dark:text-slate-400">كنحسبو تأثير الخسائر...</p> : null}
        {!isImpactLoading && impactData.is_impossible ? (
          <p className="font-semibold text-rose-400">مستحيل توصل للهدف بهاد الخسائر</p>
        ) : null}
        {!isImpactLoading && !impactData.is_impossible ? (
          <p className="text-amber-600 dark:text-amber-300">
            تأخير بسبب الخسائر: <span className="font-semibold">{impactData.delay_months || 0} شهر</span>
          </p>
        ) : null}
        ...
        {isAchieved ? <p className="font-semibold text-emerald-400">هدف محقق، برافو 🎉</p> : null}
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2 text-sm">
        ...
        <button onClick={() => onEdit(goal)} className="rounded-md bg-slate-800 px-3 py-2">
          تعديل
        </button>
        <button
        ...
        <button onClick={() => onDelete(goal.id)} className="rounded-md bg-rose-600 px-3 py-2">
          حذف
        </button>
