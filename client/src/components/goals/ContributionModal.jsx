...
export default function ContributionModal({ goal, isOpen, onClose, onSubmit, isSubmitting, budget }) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (Number(amount) > budget) {
      setError("المبلغ يتجاوز الميزانية المتاحة");
      return;
    }
    ...
    <div className="fixed inset-0 z-30 flex items-center justify-center bg-black/60 p-4">
      <form onSubmit={handleSubmit} className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-5 text-slate-900 shadow-xl dark:border-slate-800 dark:bg-slate-900 dark:text-slate-100">
        ...
        <div className="flex gap-2">
          ...
          >
            {isSubmitting ? "جارٍ الحفظ..." : "تأكيد المساهمة"}
          </button>
